...
def listPaie(idDepense, idGroupe):
    res= connexion.cursor.execute("""
        ...
        INNER JOIN depense ON depense.id = paiement.idDepense WHERE depense.id = ? AND depense.idGroupe = ?
    """, (idDepense, idGroupe))
    res = cursor.fetchall()

    os.system('clear' if os.name == 'posix' else 'cls')
    style.showStyledTitleCyan("Les Paiements")
    if not res:
    ...
    for i, paiement in enumerate(res, start=1):
        ...
        print(f"Montant payé         : {paiement[1]} FCFA")
